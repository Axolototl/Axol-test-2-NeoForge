...
public class ReturnHomeCommand {
    ...
    private int execute(CommandContext<CommandSourceStack> context) {
        ...
        boolean hasHomepos = player.getPersistentData().getIntArray("axoltest2.homepos").length != 0;

        if(hasHomepos) {
            int[] playerPos = player.getPersistentData().getIntArray("axoltest2.homepos");
